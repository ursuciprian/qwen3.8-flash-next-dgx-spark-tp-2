...
class ModelOptMixedPrecisionConfig(ModelOptQuantConfigBase):
    ...
    def _quantized_layer_group_size(self, prefix: str) -> int | None:
        """group_size of the quantized_layers entry that covers prefix."""
    ...
    def _quantized_layer_prefix_candidates(prefix: str) -> tuple[str, ...]:
        ...
        # Kai/community 2026-09-05: MTP draft layers are numbered from
    ...
    def get_quant_method(
        ...
        if isinstance(layer, RoutedExperts):
            ...
            if quant_algo in ("FP8_BLOCK_SCALES", "FP8_BLOCK", "FP8_PB_WO"):
                # Kai/community 2026-09-05: NVIDIA's Qwen3.8-Flash-Next-NVFP4 stores the MTP
                ...
                #
                # ursuciprian 2026-09-06: NVIDIA commit fc694b54 ("Fix MTP serving metadata
                # and instructions", 2026-09-05 23:36 UTC) renamed the MTP experts' algo in
                # config.json from FP8_BLOCK_SCALES to FP8_PB_WO. hf_quant_config.json still
                # says FP8_BLOCK_SCALES; the resolver reads config.json first. The two names
                # are the same tensor layout - 128x128 block-static FP8 weight, see
                # KFp8Block128 ("PbWo") in this file, which handles it for Linear but not
                # RoutedExperts. Every community overlay predates
                # that commit and dispatches only on the old name, so on fc694b54 the MTP
                # experts fall through to None and load fails with
                #   "has no parameter 'w2_weight_scale_inv'".
                # Accepting FP8_PB_WO here routes it to the same Fp8MoEMethod block path.
