...
class Qwen3_8FlashNextMTP(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        if draft_vocab_path:
            ...
            # device context. Boot1's fix only matched torch.arange's device
            # to target_ids (both stayed CPU), so draft_id_to_target_id was
            # silently CPU-resident the whole time; that only surfaces once
            # something actually indexes it with GPU indices (d2t[top] in
            # get_top_tokens below), which boots 1-3 never reached because
            # they crashed earlier in __init__/load_weights. Move to this
            # rank's real device (self.model's params are already placed by
            # the surrounding `with target_device:` context in
            # base_loader.py's initialize_model) before building the buffer.
            ...
            # The reduced-vocab draft head fully replaces the NVFP4-quantized
            # primary lm_head on the decode hot path (get_top_tokens routes
            # through draft_lm_head; speculator.py only falls back to
            # compute_logits()/primary lm_head when get_top_tokens is absent
            # -- see spec_decode/speculator.py:374-388). Keeping the primary
            # head NVFP4-quantized here only breaks _populate_draft_lm_head:
            # Nvfp4OnlineLinearMethod packs 2 4-bit values/byte along the
            # hidden dim, so its weight physically stores as
            # [vocab_local, hidden/2] uint8 (confirmed via DV-DEBUG boot:
            # lm_head.weight.shape=(124160, 1280) dtype=uint8 vs the
            # unquantized draft_lm_head's (23584, 2560) bf16 -- not a TP
            # hidden-sharding issue, a quantization packing format mismatch).
            # Force it off so both heads are plain bf16 with matching hidden
            # dims, and _populate_draft_lm_head's gather/index_select/copy
            # works on real bf16 values instead of packed fp4 bytes.
