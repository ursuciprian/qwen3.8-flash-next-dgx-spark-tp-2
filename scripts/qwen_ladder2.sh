...
boot() {  # boot <recipe> <name>: 0 healthy, 1 failed, 2 retry-worthy
  ...
  for _ in $(seq 1 180); do
    ...
    docker logs "$C" 2>&1 | grep -aqE "Engine core initialization failed|WorkerProc failed to start|Traceback" && break
    sleep 20
  done
  C=$(docker ps -q | head -1); [ -n "$C" ] && docker logs "$C" > "$OUT/$2-serve.log" 2>&1
...
for rec in "$@"; do
  ...
  if [ $rc != 0 ]; then say "$name never healthy"; grep -aE "Error|error:" "$OUT/$name-serve.log" 2>/dev/null | grep -viE "use_fast|deprecated" | tail -2 | cut -c1-200 | tee -a "$LOG"; sparkrun stop --all >/dev/null 2>&1; continue; fi
  C=$(docker ps -q | head -1); docker logs "$C" > "$OUT/$name-serve.log" 2>&1
  ...
  if [ "$runtime" = "sglang" ]; then
    EXTRA='{"return_token_ids":false,"chat_template_kwargs":{"enable_thinking":false}}'
  else
